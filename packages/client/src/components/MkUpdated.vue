...
<MkModal ref="modal" :z-priority="'middle'" @click="$refs.modal.close()" @closed="$emit('closed')">
	<div class="ewlycnyt">
		<div class="title"><MkSparkle>{{ i18n.ts.speechkaUpdated }}</MkSparkle></div>
...
const whatIsNew = () => {
	modal.value.close();
	window.open(`https://github.com/Evgen0922/speechka-devreleases/tag/${version.replace(/\./g, '-')}`, '_blank');
};
</script>
